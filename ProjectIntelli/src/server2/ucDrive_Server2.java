...
import static server2.ucDrive_Server2.clients;

// Para comunicar com os clientes
public class ucDrive_Server2 {
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        try {
            ...
            String p = System.getProperty("user.dir") + "\\config_server2.txt";
        ...
        while (true) {
            try (DatagramSocket aSocket = new DatagramSocket(UDPPort)) {
                ...
                try {
                    Scanner sc = new Scanner(new File(System.getProperty("user.dir") + "\\clients2.txt"));
                    String[] c;
                    String line;
                    ...
                    while (sc.hasNextLine()) {
                        ...
                        if (c.length == 7) {
                            clients.add(new server2.Client_Class(c[0], c[1], server_home + "\\" + c[6], c[2], c[3], c[4], c[5]));
                        } else {
                            clients.add(new server2.Client_Class(c[0], c[1], server_home + "\\" + c[0], c[2], c[3], c[4], c[5]));
                        }
                    }
                    sc.close();
                ...
                //try (DatagramSocket filesocket = new DatagramSocket(UDPPort)) {

                try (ServerSocket listenSocket = new ServerSocket(serverPort)) {

                    System.out.println("Socket Datagram à escuta no porto " + UDPPort);
                    new server2.UDPConnection(aSocket, otherUDPport, thisUDPport);

                    while (true) {
                        Socket clientSocket = listenSocket.accept(); // BLOQUEANTE
                        System.out.println("CLIENT_SOCKET (created at accept())=" + clientSocket);
                        new Connection(clientSocket, aSocket, otheraddress, otherUDPport);
                    }
                ...
                try (DatagramSocket dsf = new DatagramSocket(Integer.parseInt(otherfilePort))) {
                    new server2.FileUDPSecondary(otheraddress, Integer.parseInt(otherfilePort), dsf);
            ...
            try {
                ...
                if(!(ds.isClosed())) {
                    ...
                    for (int i=0; i<arrays.length; i++) {
                        if (i!=0)   newp.append("\\");
                        if (arrays[i].equals("server")) {
                            newp.append("server2");
                        }
                        else if (arrays[i].equals("clients1.txt")) {
                            newp.append("clients2.txt");
                        }
                        else {
...
class UDPConnection extends Thread {
    ...
    public UDPConnection(DatagramSocket aSocket, String otheraddress, String otherport) {
        UDPSocket = aSocket;
        oaddress = otheraddress;
        oport = Integer.parseInt(otherport) + 2;
        this.start();
    }
...
class Connection extends Thread {
    ...
    private synchronized void update_clients(String otheraddress, int otherUDPPort, String server_home) throws IOException {
        ...
        String p = System.getProperty("user.dir") + "\\clients2.txt";
        ...
        try (DatagramSocket ds = new DatagramSocket()) {
            ...
            String namef = System.getProperty("user.dir") + "\\clients2.txt";
            byte[] buf = namef.getBytes();
            DatagramPacket dp = new DatagramPacket(buf, buf.length, ia, otherUDPPort);
            ds.send(dp);

            long size = Files.size(Path.of(System.getProperty("user.dir") + "\\clients2.txt"));
            ...
            File real_file = new File(String.valueOf(Path.of(System.getProperty("user.dir") + "\\clients2.txt")));
